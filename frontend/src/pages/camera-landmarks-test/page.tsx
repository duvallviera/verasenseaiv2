...
export default function CameraLandmarksPage() {
  ...
  const drawLandmarksOnCamera = useCallback(async () => {
    ...
    // Debug info overlay - smaller and less intrusive
    ...
    // Small debug overlay - top right corner only
    ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
    ctx.fillRect(canvas.width - 120, 10, 110, 40);
    ctx.fillStyle = '#00ff00';
    ctx.font = 'bold 12px Arial';
    ctx.fillText('Canvas OK', canvas.width - 110, 25);
    ctx.fillText(`${canvas.width}x${canvas.height}`, canvas.width - 110, 40);
    
    // Always show Smile text - iPhone compatible
    ...
    try {
      ...
      if (detections.length > 0) {
        ...
        if (resizedDetection.landmarks) {
          ...
          // Enhanced landmark drawing with better accuracy and visibility
          ctx.shadowColor = '#ff0000';
          ctx.shadowBlur = 3; // Subtle glow for better performance
          
          // Draw each landmark point with enhanced visibility
          if (landmarks.positions && landmarks.positions.length > 0) {
            landmarks.positions.forEach((point: any, index: number) => {
              // Color-code landmarks by facial feature for better tracking
              let landmarkColor = '#ff0000'; // Default red
              let landmarkSize = 4; // Base size
              
              // Color-code by facial feature
              if (index >= 0 && index <= 16) {
                landmarkColor = '#00ff00'; // Green for jaw
                landmarkSize = 5;
              } else if (index >= 17 && index <= 26) {
                landmarkColor = '#ffff00'; // Yellow for eyebrows
                landmarkSize = 4;
              } else if (index >= 27 && index <= 35) {
                landmarkColor = '#00ffff'; // Cyan for nose
                landmarkSize = 5;
              } else if (index >= 36 && index <= 47) {
                landmarkColor = '#ff8000'; // Orange for eyes
                landmarkSize = 6;
              } else if (index >= 48 && index <= 67) {
                landmarkColor = '#ff0080'; // Pink for mouth
                landmarkSize = 4;
              }
              
              // Draw glowing circle with feature-specific color
              ...
              ctx.beginPath();
              ctx.arc(point.x, point.y, Math.max(1, landmarkSize - 2), 0, 2 * Math.PI);
              ctx.fill();
              
              // Add point numbers for key landmarks only
              if (index % 10 === 0 || index === 0 || index === 16 || index === 27 || index === 36 || index === 42 || index === 48) {
                ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';
                ctx.fillRect(point.x + landmarkSize + 2, point.y - 8, 15, 10);
                ctx.fillStyle = '#ffffff';
                ctx.font = 'bold 8px Arial';
                ctx.fillText(index.toString(), point.x + landmarkSize + 4, point.y - 1);
              }
            });
          }
          ...
          // Enhanced face mesh with better accuracy and visibility
          ctx.lineWidth = 2; // Optimal line thickness
          ctx.shadowBlur = 3; // Subtle glow effect
          
          // Draw jaw line (points 0-16) with enhanced visibility
          ctx.strokeStyle = '#00ff00'; // Bright green
          ctx.shadowColor = '#00ff00';
          ctx.beginPath();
          for (let i = 0; i <= 16; i++) {
          ...
          // Draw nose (points 27-35) with enhanced lines
          ...
          // Add additional mesh lines for better face tracking accuracy
          // Connect key facial features with enhanced visibility
          ctx.strokeStyle = '#ff00ff';
          ctx.lineWidth = 1;
          ctx.shadowColor = '#ff00ff';
          ctx.shadowBlur = 2;
          
          // Connect eyes to nose for better tracking
          if (landmarks.positions[36] && landmarks.positions[27]) {
            ctx.beginPath();
            ctx.moveTo(landmarks.positions[36].x, landmarks.positions[36].y);
            ctx.lineTo(landmarks.positions[27].x, landmarks.positions[27].y);
            ctx.stroke();
          }
          
          if (landmarks.positions[42] && landmarks.positions[27]) {
            ctx.beginPath();
            ctx.moveTo(landmarks.positions[42].x, landmarks.positions[42].y);
            ctx.lineTo(landmarks.positions[27].x, landmarks.positions[27].y);
            ctx.stroke();
          }
          
          // Connect nose to mouth for better face structure
          if (landmarks.positions[27] && landmarks.positions[48]) {
            ctx.beginPath();
            ctx.moveTo(landmarks.positions[27].x, landmarks.positions[27].y);
            ctx.lineTo(landmarks.positions[48].x, landmarks.positions[48].y);
            ctx.stroke();
          }
  ...
  useEffect(() => {
    ...
    // Don't disable landmarks when not in capture phase - let user control it
  }, [verificationFlow.phase]);
  
  // Enable landmarks when camera is active
  useEffect(() => {
    if (cameraActive) {
      console.log('📱 Enabling landmarks for camera testing');
      setShowLandmarksOnCamera(true);
  ...
  const captureReferencePhoto = async () => {
    try {
      console.log('📸 Capture reference photo button clicked');
      ...
      if (!videoRef.current || !cameraActive) {
        console.error('❌ Cannot capture: video not ready or camera not active');
        alert('Camera not ready. Please start the camera first.');
        return;
      }
      
      if (!modelsLoaded) {
        console.error('❌ Cannot capture: models not loaded');
        alert('AI models not ready. Please wait for models to load.');
        return;
      }
      
      if (referencePhotos.length >= 3) {
        console.error('❌ Cannot capture: maximum photos reached');
    ...
    console.log('🖼️ Creating image element for face detection...');
    const img = new Image();
    ...
    console.log('🎯 Face descriptor generated:', !!faceDescriptor);
    
    const detections = await (faceapi.detectSingleFace(img, new faceapi.SsdMobilenetv1Options()) as any)
      .withFaceLandmarks();
    console.log('👤 Face detections:', detections ? 'Found' : 'None');
    ...
    console.log('✅ Face detected successfully, processing...');
    ...
    console.log('💾 Saving reference photo to state...');
    setReferencePhotos(prev => [...prev, referencePhoto]);
    
    console.log(`✅ Reference photo ${currentCaptureIndex}/3 captured successfully`);
    console.log('📊 Updated reference photos count:', referencePhotos.length + 1);
    
    if (currentCaptureIndex < 3) {
      console.log('📸 Moving to next capture index...');
      setCurrentCaptureIndex(prev => prev + 1);
    } else {
      // All 3 reference photos captured
      console.log('🎉 All 3 reference photos captured! Moving to validation phase...');
      ...
      alert('Error capturing photo: ' + (error as Error).message);
    } finally {
    ...
    <div style={styles.page}>
      ...
      <div style={styles.stage}>
        <div style={styles.videoWrap}>
          ...
          {/* Transparent overlay canvas for landmarks/box */}
          <canvas ref={overlayRef} style={styles.overlay} />
          
          {/* Dedicated landmarks overlay canvas */}
          <canvas 
                ...
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '12px' }}>
                  ...
                  <button
                    style={{
                      ...
                      color: 'white',
                      border: 'none',
                      cursor: isCapturing || (!cameraActive || !modelsLoaded || referencePhotos.length >= 3) ? 'not-allowed' : 'pointer',
                      fontWeight: '600',
                      fontSize: '14px',
                      opacity: isCapturing ? 0.8 : 1
                    }}
                    onClick={captureReferencePhoto}
                    disabled={isCapturing || !cameraActive || !modelsLoaded || referencePhotos.length >= 3}
                  >
                    {isCapturing ? '⏳ Capturing...' : `📸 Capture Reference Photo ${currentCaptureIndex}/3`}
                  </button>
            ...
            <div>
              ...
              <div style={{ 
                ...
                <div style={{ color: '#22c55e', fontWeight: '600', marginBottom: '8px' }}>
                  🎉 Ready for Profile Photo Validation
                </div>
                <div style={{ color: 'rgba(255, 255, 255, 0.8)', fontSize: '14px' }}>
                  You can now upload profile photos to verify they match your reference images.
                </div>
              </div>
            </div>
          )}
